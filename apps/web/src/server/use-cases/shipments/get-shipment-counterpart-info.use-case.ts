...
export async function getShipmentCounterpartInfo(
  ...
  if (principalRole === 'CUSTOMER') {
    ...
    const contact = await repos.carrierProfileRepo.findContactInfoByUserId(accepted.carrierId)
    return {
      success: true,
      info: {
  ...
  const customerProfile = await repos.customerProfileRepo.findUserIdById(shipment.customerId)
  ...
  const contact = await repos.customerProfileRepo.findContactInfoByUserId(customerProfile.userId)
  return {
    success: true,
    info: {
