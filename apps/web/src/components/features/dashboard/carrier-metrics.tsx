...
import { useCarrierDashboardMetrics } from '~/graphql/hooks/use-carrier-dashboard-metrics'
import { formatPriceInCents } from '~/lib/format-price'

export function CarrierMetrics() {
  const { data: metrics, isLoading } = useCarrierDashboardMetrics()
  ...
  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      ...
      <MetricCard
        label="Total ganho"
        value={formatPriceInCents(metrics.totalEarnedInCents ?? 0)}
        icon={Wallet}
